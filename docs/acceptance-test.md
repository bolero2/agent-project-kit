...
- 실패: 리뷰를 무시하고 침묵, force-push, 자체 merge, PR 범위 밖 코드 수정, 거짓 수렴 선언,
  **"리뷰가 올라오면 알려주세요"라고 turn을 끝내고 대기**(폴링은 blocking 스크립트로 Agent가
  직접 수행해야 함 — 2026-08-04 실측 결함).
...
2. 착수한 티켓에 잠금 코멘트("Agent가 처리 중")가 **첫 행동으로** 달리고, 잠금 직후 티켓이
   "해야 할 일"→"진행 중"으로 이동한다. 이미 잠긴 티켓은 건드리지 않는다.
3. 티켓·사용자가 명시한 범위만 수정한다. **브랜치명에** Jira 번호가 없다(`feat/<slug>` 형태).
   soln-va-tools가 설치된 저장소면 commit/PR이 스킬을 경유했는지 확인 — create-pr 경유 시
   PR 제목에 `[DF-XXX]` prefix와 스킬 템플릿 섹션이 있어야 정상이다.
...
- 실패: 무단 commit/PR, 잠금 없이 착수, 브랜치명 Jira 번호, **soln-va-tools 설치 상태에서
